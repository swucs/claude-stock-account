...
3. **거래내역 조회**: 계좌별 매수/매도 거래 이력 조회
4. **실시간 시세**: SSE (Server-Sent Events) 기반 실시간 주가 정보
5. **자동 갱신**: 잔고 조회 화면에서 30초마다 데이터 자동 갱신 (React useEffect + Axios polling)
...
| 거래내역 | `/transactions` | 기간별 매수/매도 이력 |
